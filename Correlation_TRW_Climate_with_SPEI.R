############################################################################################################
# mean TRW vs monthly & seasonal climate correlations
# Spearman only
...
out_dir <- "E:/FAU master/Master Thesis/Results/TRW Baft correlation/New_raw_final/spearman"
...
# Read mean TRW data
# -----------------------------
data_crn1 <- read_excel(trw_path)
colnames(data_crn1)[1:2] <- c("Year", "mean_TRW")

data_crn1 <- as.data.frame(data_crn1)

data_crn1$Year <- as.numeric(data_crn1$Year)
data_crn1$mean_TRW <- as.numeric(data_crn1$mean_TRW)
...
removed_response_years <- data_crn1$Year[is.na(data_crn1$mean_TRW)]

if (length(removed_response_years) > 0) {
  cat("\nRemoved years with missing mean_TRW from response:\n")
  print(removed_response_years)
  
  write.csv(
    data.frame(removed_years_missing_mean_TRW = removed_response_years),
    file = file.path(out_dir, "removed_years_missing_mean_TRW.csv"),
    row.names = FALSE
  )
}

data_crn1 <- data_crn1[!is.na(data_crn1$mean_TRW), ]
...
write.csv(
  ...
  file = file.path(out_dir, "mean_TRW_response_used.csv"),
  row.names = FALSE
)
...
save_custom_heatmap <- function(res_obj, prefix, out_dir, approach, variable, cor_method) {
  ...
  p <- ggplot(
    ...
    labs(
      title = paste0(approach, ": mean TRW vs ", variable, " (", cor_method, ")"),
...
for (v in climate_vars) {
  ...
  for (cm in cor_methods) {
    ...
    prefix_m <- paste0("MONTHLY_mean_TRW_vs_", safe_v, "_", cm)
    ...
    prefix_s <- paste0("SEASONAL1to8_mean_TRW_vs_", safe_v, "_", cm)
...
write.csv(
  all_window_results,
  file = file.path(out_dir, "ALL_TRW_correlation_windows_with_pvalues.csv"),
  row.names = FALSE
)

write.csv(
  all_significant_results,
  file = file.path(out_dir, "ALL_TRW_significant_results_p_lt_0.05.csv"),
  row.names = FALSE
)

write.csv(
  all_significant_results[all_significant_results$Significant_p01, ],
  file = file.path(out_dir, "ALL_TRW_significant_results_p_lt_0.01.csv"),
  row.names = FALSE
)

write.csv(
  best_window_summary,
  file = file.path(out_dir, "ALL_TRW_best_window_per_variable_and_approach.csv"),
  row.names = FALSE
)
...
cat("\nMain summary files:\n")
cat("1) ALL_TRW_correlation_windows_with_pvalues.csv\n")
