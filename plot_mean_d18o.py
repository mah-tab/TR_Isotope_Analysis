...
OUT_PNG_1 = os.path.join(
    OUT_DIR,
    "d18o_per_year_samples_plus_mean.png"
)

OUT_PNG_2 = os.path.join(
    OUT_DIR,
    "d18o_mean_plus_minus_std.png"
)
...
def main():
    ...
    df["std_d18O"] = df[SAMPLES].std(
        axis=1,
        skipna=True
    )

    df["mean_d18O_complete"] = df[SAMPLES].mean(
        axis=1,
        skipna=False
    )

    df["std_d18O_complete"] = df[SAMPLES].std(
        axis=1,
        skipna=False
    )
    ...
    # Show a tick + label every 5 years
    xticks = list(range(year_min, year_max + 1, 5))

    # Always also show the final year as a tick + label
    ...
    # PLOT 1
    # ============================================================
    # Keep original plot size; legend is added outside the plot
    ...
    plt.xlabel(
        "Year",
        fontsize=XLABEL_FONTSIZE+2
    )

    plt.ylabel(
        YLABEL,
        fontsize=YLABEL_FONTSIZE+2
    )

    plt.title(
        "δ$^{18}$O per Year (Tree-Ring Samples) + Mean",
        fontsize=TITLE_FONTSIZE+2
    )

    # Legend outside plot on the upper right
    plt.legend(
        title="Series",
        fontsize=LEGEND_FONTSIZE+2,
        title_fontsize=LEGEND_TITLE_FONTSIZE+2,
        loc="upper left",
    ...
    plt.xticks(
        xticks,
        fontsize=XTICK_FONTSIZE+2
    )

    plt.yticks(
        fontsize=YTICK_FONTSIZE+2
    )
    ...
    plt.title(
        "δ$^{18}$O per Year — Mean ± Standard Deviation",
        fontsize=TITLE_FONTSIZE
    )

    # Legend inside plot at bottom left
    plt.legend(
        title="Series",
    ...
    mean_chron_df = mean_chron_df.dropna(
        subset=["Year"]
    ).copy()
    ...
    mean_chron_year_min = int(
        mean_chron_df["Year"].min()
    )

    mean_chron_year_max = int(
        mean_chron_df["Year"].max()
    )

    # Show a tick + label every 5 years
    mean_chron_xticks = list(
        range(
            mean_chron_year_min,
            mean_chron_year_max + 1,
            5
        )
    )

    # Always also show the final year as a tick + label
    if mean_chron_year_max not in mean_chron_xticks:
        mean_chron_xticks.append(mean_chron_year_max)

    mean_chron_xticks = sorted(mean_chron_xticks)
    ...
    plt.title(
        "δ$^{18}$O per Year — Mean Chronology ± Standard Deviation",
        fontsize=TITLE_FONTSIZE
    )
    ...
    plt.savefig(
        OUT_PNG_MANUAL_MEAN_CHRON_GAPS,
        dpi=500,
        bbox_inches="tight"
    )
